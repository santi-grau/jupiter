...
attribute vec2 la; // lookAt texture
...
void main() {
	...
	// oscillation
	// p = curlNoise( vec3( position.x, position.y, position.z + settings.x ) ) * 10.0;

	// pointSize
	float pNoise = ( snoise( vec3( position.x / 0.01, position.y / 0.01, 0.0 ) ) + 1.0 ) / 2.0;
